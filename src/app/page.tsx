  ...
  return (
    <div className="h-screen w-full relative bg-[#111111]">
      ...
      <div className="absolute inset-0 md:relative md:h-screen">
        {MapComponent}
        {/* Desktop Controls */}
        <div className="hidden md:flex absolute top-4 right-4 items-center gap-4 z-20">
          {session ? (
            <>
              <button
                onClick={() => setIsAddItemModalOpen(true)}
                className="px-4 py-2 bg-[#e6e6e6] text-black rounded-lg text-sm font-medium hover:bg-gray-300 transition-colors shadow-lg"
              >
                Add Item
              </button>
              <button
            ...
            <button
              onClick={() => signIn('google')}
              className="px-4 py-2 bg-white text-gray-800 rounded-lg text-sm font-medium hover:bg-gray-100 transition-colors shadow-lg flex items-center gap-2"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
