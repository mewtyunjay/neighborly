...
export default function AddItemModal({ isOpen, onClose, fridges }: AddItemModalProps) {
  ...
  const categories = ['food', 'medicine', 'utilities'];
      ...
      <div className="bg-[#111111] rounded-2xl w-full max-w-[600px] max-h-[80vh] overflow-hidden border border-gray-800 shadow-xl">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(80vh-80px)]">
          <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div
                    ...
                    className={`p-4 rounded-xl transition-all duration-200 cursor-pointer border ${
                      selectedFridge?.id === fridge.id
                        ...
                        : 'hover:bg-[#1D1D1D] border-gray-800/50 hover:border-emerald-500/20'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-gray-100 font-medium">{fridge.name}</h3>
                        <p className="text-sm text-gray-500 mt-1">{fridge.address}</p>
                      </div>
                      <span className="text-sm text-gray-400">{fridge.distance}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            >
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-gray-400 group-hover:text-gray-300">
                  {isProcessingImage ? 'Processing...' : 'Take a photo to auto-fill details'}
                </span>
              </div>
            </button>
            ...
            <button
              type="submit"
              disabled={!selectedFridge || parseInt(formData.quantity) === 0}
